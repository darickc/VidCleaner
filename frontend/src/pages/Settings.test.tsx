...
function stub(extra: Record<string, unknown> = {}) {
  return mockApi({
    routes: {
      "/settings": SETTINGS,
      "/path-mappings": [{ app: "sonarr", from_prefix: "/tv", to_prefix: "/media/tv" }],
...
describe("settings page", () => {
  it("shows the stored values", async () => {
    ...
    await waitFor(() => expect(screen.getByLabelText("Sonarr URL")).toBeDefined());
  ...
  it("sends only the fields that changed", async () => {
    ...
    await waitFor(() => {
      const patch = api.calls.find((c) => c.method === "PATCH" && c.path === "/settings");
  ...
  it("tests an integration with the values on screen", async () => {
    const api = stub({
      "/integrations/sonarr/test": { app: "sonarr", ok: true, version: "4.0.1", detail: "", latency_ms: 12 },
    });
    renderApp(<SettingsPage />);

    await userEvent.click(await screen.findByRole("button", { name: "Test Sonarr" }));

    await waitFor(() => expect(screen.getByText(/ok · 4\.0\.1 · 12ms/)).toBeInTheDocument());
    expect(api.calls.find((c) => c.path === "/integrations/sonarr/test")?.body).toEqual({
  ...
  it("reports a failed test rather than throwing it away", async () => {
    ...
    await userEvent.click(await screen.findByRole("button", { name: "Test Jellyfin" }));
  ...
  it("shows the webhook URL and token to paste", async () => {
    ...
    expect(screen.getAllByText(/X-VidCleaner-Token: s3cret/).length).toBeGreaterThan(0);
  });
  ...
  it("installs the webhook in the arr on request", async () => {
    ...
    await userEvent.click(await screen.findByRole("button", { name: "Add to Sonarr" }));
    await waitFor(() => expect(screen.getByText("created")).toBeInTheDocument());
    expect(api.calls.some((c) => c.path.startsWith("/webhooks/install"))).toBe(true);
  });

  it("edits and saves path mappings", async () => {
    const api = stub({ "/path-mappings": [{ app: "sonarr", from_prefix: "/tv", to_prefix: "/media/tv" }] });
    renderApp(<SettingsPage />);
    ...
    await waitFor(() => {
      ...
      expect(put?.body).toEqual([{ app: "sonarr", from_prefix: "/data/tv", to_prefix: "/media/tv" }]);
    });
  });
  ...
  it("adds and removes a mapping row", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: "Remove mapping 2" }));
